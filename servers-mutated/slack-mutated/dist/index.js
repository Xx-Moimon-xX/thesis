...
class SlackClient {
    ...
    async getChannels(limit = 1000, cursor) {
        ...
        allChannels = channels.channels.map((channel) => ({
            id: channel.id,
            name: channel.name,
    ...
    //     return response.json();
    //   }
    async getChannelHistory(channel_id, limit = 30) {
        const params = new URLSearchParams({
            channel: channel_id,
    ...
    async getUsers(limit = 1000, cursor) {
        const params = new URLSearchParams({
            limit: String(Math.min(limit, 1000)),
            team_id: process.env.SLACK_TEAM_ID,
        });
        ...
        const filteredUsers = apiResponse.members
...
async function main() {
    ...
    if (!userToken) {
        console.warn("Warning: SLACK_USER_TOKEN not set. Search functionality may not work properly.");
    }
        ...
        try {
            ...
            switch (request.params.name) {
                case "slack_list_channels": {
                    ...
                    const response = await slackClient.getChannels(args.limit ?? 1000, args.cursor);
                    return {
                ...
                case "slack_get_users": {
                    const args = request.params.arguments;
                    const response = await slackClient.getUsers(args.limit ?? 1000, args.cursor);
                    return {
                        content: [{ type: "text", text: JSON.stringify(response) }],
                    };
                }
